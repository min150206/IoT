...
ESP32_IP = "192.168.137.50"   # change to your ESP32 IP (must match the IP printed on its Serial Monitor)
ESP32_PORT = 80
...
NO_PLATE_CLOSE_DELAY = 3     # seconds with NO plate box visible before auto-closing the gate
...
def signal_closed():
    """Red LED - gate closed (idle state)."""
    send_command("closed")
...
def main():
    ...
    # Tracks what the ESP32 is currently showing, so we only send a command
    # when the state actually needs to change (and so auto-close always works,
    # even if the gate never reached "open").
    current_signal = "closed"   # one of: "closed", "checking", "open"

    last_seen_plate_time = None   # last frame timestamp where ANY plate box was seen
    ...
    while True:
        ...
        if read_this_frame is not None:
            ...
            # nothing read this frame, drop the candidate (does not affect gate state)
            candidate_plate = None
            candidate_since = None

        # ---------- Auto-close: no plate BOX seen for NO_PLATE_CLOSE_DELAY seconds ----------
        # Runs whenever the gate is not already closed, regardless of whether
        # it reached "open" or got stuck at "checking".
        if current_signal != "closed":
            reference_time = last_seen_plate_time if last_seen_plate_time is not None else now
            elapsed = now - reference_time
            if elapsed > NO_PLATE_CLOSE_DELAY:
                print(f"[GATE] No plate detected for {NO_PLATE_CLOSE_DELAY}s, closing gate.")
                signal_closed()
                current_signal = "closed"
                last_seen_plate_time = None
                candidate_plate = None
                candidate_since = None
